...
var ethereum = require('ethereumjs-utils')
...
Stealth.prototype.toEthBuffer = function () {
  return bconcat([
    this.version,
    0, // options
    this.scanEthPubKey,
    1, // number of payload keys, only 1 atm
    this.payloadEthPubKey,
    1, // number of sigs, only 1 atm
    0 // prefix length, not supported (actually, don't even know what the hell it is)
  ])
}
...
Stealth.fromBuffer = function (buffer) {
  ...
  // console.log(payloadPubkeys[0].toString(''));
...
Stealth.fromEthBuffer = function (buffer) {
  const pkLen = 64
  var pos = 0
  var version = buffer.readUInt8(pos++)
  var options = buffer.readUInt8(pos++)
  ...
  var scanEthPubKey = buffer.slice(pos, pos += pkLen)
  var nPayloadPubkeys = buffer.readUInt8(pos++)

  var payloadEthPubkeys = []
  for (var i = 0; i < nPayloadPubkeys; i++) {
    payloadEthPubkeys.push(buffer.slice(pos, pos += pkLen))
  }
  ...
  return new Stealth({
    payloadPubKey: payloadEthPubkeys[0],
    payloadEthPubKey: payloadEthPubkeys[0],
    scanPubKey: scanEthPubKey,
    scanEthPubKey: scanEthPubKey,
    version: version
  })
}
...
Stealth.fromRandom = function (config) {
  ...
  var payloadEthPubKey = ethereum.privateToPublic(payloadPrivKey);
  var scanPubKey = ecparams.G.multiply(BigInteger.fromBuffer(scanPrivKey)).getEncoded(true)
  var scanEthPubKey = ethereum.privateToPublic(scanPrivKey);
  return new Stealth({
    ...
    payloadEthPubKey: payloadEthPubKey,
    version: version
  })
}
...
Stealth.prototype.genEthPaymentPubKeyHash = function (senderPrivKey) {
  var kdf = crypto.hmacSha256
  
  // var Ap = Point.decodeFrom(ecparams, this.scanEthPubKey)
  // var A = Point.decodeFrom(ecparams, this.payloadEthPubKey)
  ethPubKey=this.scanEthPubKey
  var Ap = Point.decodeFrom(ecparams, this.scanPubKey)
  var A = Point.decodeFrom(ecparams, this.payloadPubKey)
  ...
  var pubKeyHash = E.getEncoded(true)
  return pubKeyHash
}

Stealth.prototype.genEthPaymentAddress = function (senderPrivKey) {
  var pubKeyHash = this.genEthPaymentPubKeyHash(senderPrivKey)
  var importPublic = ethereum.importPublic(pubKeyHash);
  var ethAddress = ethereum.pubToAddress(importPublic);
  return ethereum.addHexPrefix(ethAddress.toString('hex'));
}

Stealth.prototype.checkEthPaymentPubKeyHash = function (opReturnPubKey, pubKeyHashToCompare) {
  assert(this.payloadPrivKey, 'payloadPrivKey must be set if you use this method. i.e. Must be owner / receiver.')
  assert(this.scanPrivKey, 'scanPrivKey must be set if you use this method. i.e. Must be owner / receiver.')
  console.log(opReturnPubKey.length);
  if (opReturnPubKey.length !== 128) return null
  ...
  console.log(pubKeyHashToCompare.toString('hex'));
...
Stealth.fromJSON = function (json) {
  ...
  return new Stealth({
    ...
    payloadEthPubKey: new Buffer(o.payloadEthPubKey, 'hex'),
    scanPubKey: new Buffer(o.scanPubKey, 'hex'),
    scanEthPubKey: new Buffer(o.scanEthPubKey, 'hex'),
